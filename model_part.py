...
def _variable_with_weight_decay(name, shape, stddev, wd, trainable=True):
    var = _variable_on_gpu(name, shape, tf.truncated_normal_initializer(stddev=stddev), trainable)
    if wd:
...
def _variable_on_gpu(name, shape, initializer, trainable):
...
def conv2d(scope_name, inputs, shape, bias_shape, stride, padding='VALID', wd=0.0, reuse=False, trainable=True):
    with tf.variable_scope(scope_name) as scope:
        ...
        biases = _variable_on_gpu('biases', bias_shape, tf.constant_initializer(0.1), trainable)
